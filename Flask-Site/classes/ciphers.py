...
def playfair(message: str, keyword: str = '', mode: str = "encode", verbose: bool = False, results: bool = True) -> str:
    ...
    if len(message) <= 1:
        return f"[!] What am I supposed to do with that? Come back with more than {len(message)} letters..."
    if len(keyword) > 25:
        return "[!] Key should not be longer than 25 characters"
    if mode.upper() not in ["DECODE", "ENCODE"]:
        return "[!] Please enter mode=decode or mode=encode"
